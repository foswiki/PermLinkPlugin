...
sub handle_PERMLINK {
    ...
    my ( $theWeb, $theTopic ) =
      Foswiki::Func::normalizeWebTopicName( $web, $params->{topic} || $topic );
    my $theFormat = $params->{format} || '$url$md5';
    my $theURL = $Foswiki::cfg{Plugins}{PermLinkPlugin}{shortURL}
      || "%SCRIPTURL{rest}%/PermLinkPlugin/view/";
    ...
    if (
        not Foswiki::Func::checkAccessPermission(
            "VIEW", Foswiki::Func::getCanonicalUserID(),
    ...
    if ( not( $human and $md5 ) ) {
        if ($theWarn) {
            return
              '%MAKETEXT{"Error: no IDs found, please edit/save this topic"}%';
        }
        else { return q{} }
    }
...
sub write_id {
    ...
    unless ($no_check) {
        ...
        return 0
          if
            not Foswiki::Func::checkAccessPermission( "CHANGE",
...
sub rest_deploy {
    ...
    if ( not Foswiki::Func::isAnAdmin() ) {
        $session->{response}
          ->status("403 Forbidden: You need to be an admin to do that.");
    ...
    foreach my $topic (@topicList) {
        if ( write_id( $isSetWeb, $topic, undef, undef, 1 ) ) {
            ...
            $retval .=
              "<font color='red'>$isSetWeb.$topic: error</font>  <br />";
        }
    }
